...
//  TextToVoice.swift
//  Ridna Mova
//  Google TextToVoice API
//  Created by Yevhenii Ganusich on 1/15/23.
//
...
class SpeechService: NSObject, AVAudioPlayerDelegate {
    ...
    func speak(text: String, voiceType: VoiceType, language: LanguageCode, completion: @escaping () -> Void) {
        ...
        DispatchQueue.global(qos: .background).async {
            let postData = self.buildPostData(text: text, voiceType: voiceType, languageCode: language)
    ...
    private func buildPostData(text: String, voiceType: VoiceType, languageCode: LanguageCode) -> Data {
